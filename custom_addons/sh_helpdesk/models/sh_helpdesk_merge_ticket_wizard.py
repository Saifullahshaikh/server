...
class MergeTicketWizard(models.TransientModel):
    ...
    sh_user_id = fields.Many2one('res.users', string='Assigned User', domain=[
                                 ('share', '=', False)])
    ...
    sh_ticket_alarm_ids = fields.Many2many(
        'sh.ticket.alarm', string='Ticket Reminder')
    sh_helpdesk_ticket_ids = fields.Many2many(
        'sh.helpdesk.ticket', string='Tickets', readonly=True)
    sh_team_id = fields.Many2one('sh.helpdesk.team', string='Team')
    sh_user_ids = fields.Many2many('res.users', string='Assign Multi Users')
    sh_team_head_id = fields.Many2one(
        'res.users', string='Team Head', readonly=True)
    ...
    sh_select_type = fields.Selection(
        [('new', 'New'), ('existing', 'Existing')], string="Type", default="new", required=True)
    sh_existing_ticket = fields.Many2one(
        'sh.helpdesk.ticket', string="Select Ticket")
    sh_check_multi_user = fields.Boolean(
        'sh_check_multi_user', default=_default_sh_check_multi_user)
    ...
    def action_merge_tickets(self):
        ...
        for ticket in sorted_tickets:
            ...
            get_messages = self.env['mail.message'].search(
                [('res_id', '=', ticket.id)], order='id')
            ...
            get_activities = self.env['mail.activity'].search(
                [('res_id', '=', ticket.id)])
            ...
            attachment_ids_list = attachment_ids_list + \
                ticket.attachment_ids.ids if ticket.attachment_ids else []
        ...
        for ticket in self.sh_helpdesk_ticket_ids:
            if ticket.description:
                marged_disc = marged_disc + ticket.name + escape(Markup("<hr/>")) + ticket.description + escape(
                    Markup("<br/>")) if not ticket.description == '<p><br></p>' else False
