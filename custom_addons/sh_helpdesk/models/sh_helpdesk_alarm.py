...
class TicketAlarm(models.Model):
    ...
    @api.onchange('sh_reminder_unit')
    def _onchange_sh_reminder_unit(self):
        if self.sh_remind_before and self.filtered(lambda c: c.sh_reminder_unit == 'Minute(s)' and c.sh_remind_before < 5):
            raise ValidationError(
                _("Reminder Before can't set less than 5 Minutes."))
        elif self.sh_remind_before and self.filtered(lambda c: c.sh_reminder_unit == 'Second(s)' and c.sh_remind_before < 300):
            raise ValidationError(
                _("Reminder Before can't set less than 300 Seconds."))
        elif self.sh_remind_before and self.filtered(lambda c: c.sh_reminder_unit == 'Hour(s)' and c.sh_remind_before < 1):
            raise ValidationError(
                _("Reminder Before can't set less than 1 Hour."))
                ...
                for alarm in alarm_ids:
                    ...
                    if ticket_ids:
                        for ticket in ticket_ids:
                            ...
                            if deadline_date and deadline_date:
                                if alarm.type == 'popup':
                                    ...
                                    if fields.Date.today() == deadline_date.date() and deadline_date.hour == now.hour and deadline_date.minute == now.minute:
                                        ...
                                        if ticket.subject_id:
                                            message += "<strong>Subject :</strong>" + \
                                                str(ticket.subject_id.name) + \
                                                "</br>"
                                        if ticket.ticket_type:
                                            message += "<strong>Type :</strong>" + \
                                                str(ticket.ticket_type.name) + \
                                                "</br>"
                                        ...
                                        if ticket.team_id:
                                            message += "<strong>Team :</strong>" + \
                                                ...
                                                "</br>"
                                        if ticket.team_head:
                                            message += "<strong>Team Head :</strong>" + \
                                                str(ticket.team_head.name) + \
                                                "</br>"
                                        if ticket.user_id:
                                        ...
                                        model_href = str(self.env['ir.config_parameter'].sudo().get_param(
                                            'web.base.url')) + '/web#id='+str(ticket.id)+'&model=sh.helpdesk.ticket&view_type=form'
                                        ...
                                        if ticket.team_head:
                                            if ticket.team_head.partner_id.id not in partners:
                                                partners.append(
                                                    ticket.team_head.partner_id.id)
                                        if ticket.user_id:
                                        ...
                                        if len(partners) > 0:
                                            for partner_id in partners:
                                                partner = self.env['res.partner'].sudo().browse(
                                                    partner_id)
                                                notification_data_list = [partner, 'simple_notification', {'type': 'simple_notification', 'title': _(
                                                    'Ticket Reminder '+'('+str(ticket.name)+')'), 'message_is_html': message, 'message': message}]
                                    ...
                                    if ticket.team_head:
                                        if str(ticket.team_head.partner_id.email_formatted) not in email_formatted:
                                            email_formatted.append(
                                                str(ticket.team_head.email_formatted))
                                    if ticket.user_id:
                                    ...
                                    if fields.Date.today() == deadline_date.date() and deadline_date.hour == now.hour and deadline_date.minute == now.minute:
                                        reminder_template = self.env.ref(
                                            'sh_helpdesk.sh_ticket_reminder_mail_template')
