...
class HelpdeskSLAStatus(models.Model):
    ...
    sh_create_date = fields.Datetime(string='Create Date',default=fields.Datetime.now)
...
class HelpdeskTicket(models.Model):
    ...
    def action_helpdesk_ticket_merge(self):
        ...
        get_tickets = self.env['sh.helpdesk.ticket'].browse(self.env.context.get('active_ids'))
    ...
    def _compute_state_boolean(self):
        if self:
            for rec in self:
                ...
                # ----------------------------------------------------------------------
                # MANAGE (sh.helpdesk.sla.analysis) PIVOT VIEW DATA
                # ----------------------------------------------------------------------
                try:
                    if rec.sh_sla_status_ids:
                        for sla_status in rec.sh_sla_status_ids:
                            get_sla_status_analysis_record = self.env['sh.helpdesk.sla.analysis'].search(
                                [('sh_sla_status_id', '=', sla_status.id)], limit=1)
                            
                            if not get_sla_status_analysis_record:
                            
                                # REACHED DURATION ----------
                                sh_sla_status_reached = False
                                if sla_status.sh_status == 'sla_passed':
                                    reached_duration = (sla_status.sh_done_sla_date - sla_status.sh_create_date)
                                    reached_seconds = reached_duration.total_seconds()
                                    sh_reached_duration = reached_seconds / (60 * 60)

                                    if sla_status.sh_done_sla_date:
                                        sh_sla_status_reached = sla_status.sh_done_sla_date

                                else:
                                    sh_reached_duration = 0.0    
                                
                                # REACHED DURATION ---------- 

                                # LATE DURATION ----------
                                
                                if sla_status.sh_status == 'sla_failed':
                                    late_duration = (sla_status.sh_deadline - sla_status.sh_done_sla_date)
                                    late_seconds = late_duration.total_seconds()
                                    sh_late_duration = late_seconds / (60 * 60)
                                else:
                                    sh_late_duration = 0.0    
                                
                                # LATE DURATION ---------- 
                                
                                sla_analysis_id = self.env['sh.helpdesk.sla.analysis'].create(
                                    {
                                        'sh_sla_status_id': sla_status.id,
                                        'sh_sla_status_deadline': sla_status.sh_deadline,
                                        'sh_reached_duration': sh_reached_duration,
                                        'sh_sla_id':sla_status.sh_sla_id.id,
                                        'sh_sla_status_reached': sla_status.sh_done_sla_date,
                                        'sh_helpdesk_ticket_id':rec.id,
                                        'sh_late_duration': abs(sh_late_duration),
                                        'sh_ticket_stage_id':rec.stage_id.id
                                    })
                                
                                sla_deadline = rec.create_date
                                working_schedule = rec.team_id.sh_resource_calendar_id
                                if not working_schedule:
                                    sla_analysis_id.sh_sla_status_deadline = sla_deadline
                                    continue
                                if sla_status.sh_sla_id.sh_days > 0:
                                    sla_deadline = working_schedule.plan_days(
                                        sla_status.sh_sla_id.sh_days + 1,
                                        sla_deadline,
                                        compute_leaves=True)
                                    ticket_create_dt = rec.create_date
                                    sla_deadline = sla_deadline.replace(
                                        hour=ticket_create_dt.hour,
                                        minute=ticket_create_dt.minute,
                                        second=ticket_create_dt.second,
                                        microsecond=ticket_create_dt.microsecond)
                                if sla_status.sh_sla_id.sh_hours > 0:
                                    sla_analysis_id.sh_sla_status_deadline = working_schedule.plan_hours(
                                        sla_status.sh_sla_id.sh_hours, sla_deadline, compute_leaves=True)
                                sla_analysis_id.sh_sla_status_deadline = working_schedule.plan_hours(
                                    sla_status.sh_sla_id.sh_minutes, sla_deadline, compute_leaves=True)
                                ...
                                if sla_status.sh_status == 'sla_failed':
                                    ...
                                    sh_late_duration = 0.0    
                                # LATE DURATION ----------
                                get_sla_status_analysis_record.write({
                                    'sh_reached_duration':sh_reached_duration,
                                    'sh_late_duration': abs(sh_late_duration),
                                    'sh_sla_status_reached':sh_sla_status_reached,
                                    })                                
                ...
                # # --------------------------------- sh_days_to_reach - compute (_compute_days_to_reach) -------------------------------
                # sh_days_to_reach = 0.0
                # if rec.sh_sla_status_ids:
                #     for line in rec.sh_sla_status_ids:
                #         if line.sh_deadline and line.sh_done_sla_date:
                ...
                # # --------------------------------- sh_days_to_late - compute (_compute_days_to_late) -------------------------------
                # sh_days_to_late = 0.0
                # if rec.sh_sla_status_ids:
                #     for line in rec.sh_sla_status_ids:
                #         if line.sh_deadline and line.sh_done_sla_date:
    ...
    def message_post(self, **kwargs):
        if kwargs.get('subtype') != 'mail.mt_note':
                            ...
                            if len(email_to) > 1:
                                ...
                                for email in email_to:
                                    ...
                                    if partner_id:
                                        ...
                                        partner_ids.append(partner_id.id)
                        if 'cc' in kwargs and kwargs.get('cc') != '':
            ...
            elif self and kwargs and 'author_id' not in kwargs:
                if 'subtype_id' not in kwargs:
                    if self.team_id:
                        if self.team_id.alias_name and self.team_id.alias_domain:
                            email = str(self.team_id.alias_name) + \
    ...
    @api.model
    def _run_stage_overdue_ticket(self):
        for rec in self:
            if rec['sh_sla_deadline']:
                # raise UserError(rec['sh_sla_deadline']) 
                current_datetime = datetime.now()
                sla_deadline = rec['sh_sla_deadline']
                time_overdue = (current_datetime - sla_deadline).total_seconds()
                if time_overdue > 0:
